...
void	draw_rays_2D(t_cub3D *cub3D)
{
	...
	while (i < WIDTH)
	{
		...
		{
			distance_from_top = j + (line_h / 2) - (HEIGHT / 2);
			//need to change 64 to the width of the texture
			texture_y = distance_from_top * (cub3D->texture.height / line_h);
			buffer[j * WIDTH + i] = *(int *)(cub3D->texture.addr + (texture_y
						* cub3D->texture.line_length) + texture_x
					* (cub3D->texture.bits_per_pixel / 8));
			j++;
		}
		//------------------------------------------------------------------//
		i++;
...
void	draw_new_move(t_cub3D *cub3D)
{
    ...
    mlx_clear_window(cub3D->ptr_mlx, cub3D->mlx_win);
		mlx_put_image_to_window(cub3D->ptr_mlx, cub3D->mlx_win, cub3D->data.img,
				0, 0);
	}
...
int mouse_move(int x, int y, t_cub3D *cub3D)
{
  ...
  cub3D->player.retation_angle += (x - cub3D->mouse.x) * 0.001;
  cub3D->player.retation_angle = norm_angle(cub3D->player.retation_angle);
  cub3D->mouse.x = x;
  cub3D->mouse.y = y;
  return (0);
...
void	hooks_handler(t_cub3D *cub3D)
{
  ...
  mlx_hook(cub3D->mlx_win, MotionNotify, PointerMotionMask, mouse_move, cub3D);
